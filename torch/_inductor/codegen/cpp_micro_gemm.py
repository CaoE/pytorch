...
{%- endfor %}
    ...
    for (int k = 0; k < last_k_offset; k += {{block_k}}) {
        if (k + prefetch_distance < last_k_offset) {
        {%- for k_slice in range(block_k//vnni_size//8) %}
            {%- for tile_col in range(num_columns) %}
            _mm_prefetch(B + (k+prefetch_distance) * ldb + {{k_slice * num_columns * 64}} + {{tile_col * 16 * vnni_size}}, _MM_HINT_T0);
            {%- endfor %}
        {%- endfor %}
        }
        compute(k);
