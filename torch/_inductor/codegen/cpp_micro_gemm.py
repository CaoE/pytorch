...
from .common import CSEVariable, Kernel, KernelArgs, OptimizationContext, RemovedArg
...
class EpilogueBufferHandler(V.WrapperHandler):  # type: ignore[name-defined]
    def __init__(
        ...
        self.local_acc_names = []
        self.local_acc = local_acc
        for buf in self.local_acc:
            self.local_acc_names.append(buf.get_name())
...
class EpilogueBufferContext:
    ...
    def __enter__(self):
        ...
        def get_dtype(name):
            ...
            if name in self.local_acc_names:
                for buf in self.local_acc:
...
"""
    ...
    def get_common_options(self):
        ...
        return {
            ...
            "store_t": DTYPE_TO_CPP[self.input_dtype],
            "alpha": self.alpha,
...
{%- if enable_epilogue %}
    ...
    constexpr int m_end = {{num_rows}};
    constexpr int Nr = {{num_columns}} * 16;
    constexpr int n_start = 0;
    constexpr int n_end = Nr;
{%- endif %}
...
"""
    ...
    def codegen_define(self, kernel: CppTemplateKernel, fake_buffers=None, epilogue_nodes=None) -> str:
        ...
        options = {
            "declare_kernel": self.get_kernel_declaration(),
            "declare_epilogue_kernel": self.get_epilogue_kernel_declaration(),
            ...
            "enable_epilogue": epilogue_nodes is not None and len(epilogue_nodes) > 0,
            **self.get_common_options(),
        }
        self.fake_buffers = fake_buffers
        self.fake_buffers_name = []
        ...
        def epilogue_offset_placeholder():
            ...
            for node in epilogue_nodes:
                rws = node.get_read_writes()
                for write in rws.writes:
                    if 'GemmOut' in write.name and type(kernel.args.input(write.name)) != RemovedArg:
                        arg_name = kernel.args.input(write.name)
                        arg = V.graph.get_buffer(write.name)
                        with res.indent():
    ...
    def get_epilogue_kernel_declaration(self):
        options = self.get_common_options()
        ...
        **kwargs_for_extra_args,
        ...
        def arg_hook():
            ...
            for node in epilogue_nodes:
                ...
                #     arg_name = kernel.args.input(write.name)
                #     if 'GemmOut' in write.name:# and type(arg) != RemovedArg:
                #         with res.indent():
                ...
                for rd in rws.reads:
                    if 'GemmOut' not in rd.name:
                        ...
                        arg_ptr = "null"
                        if arg is not None:
                            if len(arg.get_layout().size) == 2:
                                arg_ptr = f"&({kernel.index(arg, [offsets[0], offsets[1]])})"
                            elif len(arg.get_layout().size) == 1:
        ...
        def epilogue_store_hook():
            ...
            if epilogue_store is not None:
                ...
                with res.indent():
                    # epilogue_final_store = f"if constexpr (do_epilogue) {epilogue_store.getvalue()}\n"
                    epilogue_final_store = epilogue_store.getvalue()
                    res.writeline(epilogue_final_store)
            return res.getvalue()

        kernel.render_hooks["EPILOGUE_CALL_PLACEHOLDER"] = arg_hook
        kernel.render_hooks["EPILOGUSE_STORE_PLACEHOLDER"] = epilogue_store_hook
        return res.getvalue()

    def epilogue_post_process(self, code_lines):
        import re
        # def dedup_store(code: str) -> str:
        #     store_pat = re.compile(r'\b\w+\.store\s*\(\s*Y')
        ...
        #         if i == last_idx or not store_pat.search(ln)
        #     )
        # m_x0 = re.search(r'for\s*\(\s*int64_t\s+x0\s*=.*?\)\s*{', code)
        # assert m_x0
        # m_x1 = re.search(r'for\s*\(\s*int64_t\s+x1\s*=.*?\)\s*{', code[m_x0.end():])
        # assert m_x1

        # m = re.search(r'if\s*\(\s*C10_LIKELY', code)
        # if not m:
        #     return None, code
        ...
        # block = code[brace_open + 1:end_block]
        # pattern = re.compile(r'local_acc_buf[^,]*,')
        # new_block = pattern.sub('C + (x0 * ldc + x1),', block)
        # pattern = re.compile(r'Y[^,]*,')
        # new_block = pattern.sub('Y + (x0 * ldy + x1),', new_block)
        # new_block = dedup_store(new_block)

        # new_code = (
        #     code[:m_x0.end() + m_x1.end()]
        #     + "\n" + new_block.rstrip() + "\n"
        #     + "     }\n }\n"
        # )
        # return new_code

        # find loops and replace index variables
        code_lines
        if_start = 0
        for i, line in enumerate(code_lines):
            ...
            if if_start > 0:
                if (type(line) == str and line.strip() == "}"):
                    if_end = i
                    break
                # if hasattr(line, 'line') and line.line.strip() == "}":
                #     if_end = i
                #     break
        ...
        for i, line in enumerate(inner_lines):
            ...
            if "store(Y" in line:
                inner_lines[i] = re.sub(r'Y[^,]*,', 'Y + (x0 * ldy + x1),', line)
            ...
            for node in epilogue_nodes:
                ...
                for d in read_deps:
                    if d.name in self.fake_buffers_name:
                        for buf in self.fake_buffers:
                            if "GemmOut" in buf.name and d.name == buf.name:
                                scope.local_acc.append(buf)
                                scope.local_acc_names.append(buf.name)
