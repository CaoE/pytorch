...
# def _sfdp_pattern_21(query, key, value, attn_mask, inplace_add):
#     # for T5 w/o view
#     query = query.permute([0, 2, 1, 3])
#     key = key.permute([0, 2, 1, 3])
#     value = value.permute([0, 2, 1, 3])
...
def _sfdp_pattern_t5_v1(query, key, value, attn_mask, do_view):
    # for T5 with/without view
    query = query.permute([0, 2, 1, 3])
    key = key.permute([0, 2, 1, 3])
    value = value.permute([0, 2, 1, 3])
    score = torch.matmul(query, key.permute(0, 1, 3, 2))
    ...
    score = masked_score.type_as(query)
    if do_view:
        viewd_score = score.view(
            score.size(0) * score.size(1), score.size(2), score.size(3)
        )
        score = viewd_score.view(
            score.size(0), score.size(1), score.size(2), score.size(3)
        )
    return score.float().softmax(dim=-1).type_as(query).matmul(value)

def _sfdp_pattern_22(query, key, value, attn_mask):
    return _sfdp_pattern_t5_v1(query, key, value, attn_mask, do_view=True)

def _sfdp_pattern_23(query, key, value, attn_mask):
    return _sfdp_pattern_t5_v1(query, key, value, attn_mask, do_view=False)
...
# def _sfdp_pattern_22(query, key, value, attn_mask):
#     # for T5 with inplace/outplace add and return key and value
#     query = query.permute([0, 2, 1, 3])
#     key = key.permute([0, 2, 1, 3])
#     value = value.permute([0, 2, 1, 3])
#     score = torch.matmul(query, key.permute(0, 1, 3, 2))
...
def _sfdp_pattern_t5_v2(query, key, value, attn_mask, do_view):
    # for T5 with/without view and return key and value
    query = query.permute([0, 2, 1, 3])
    key = key.permute([0, 2, 1, 3])
    value = value.permute([0, 2, 1, 3])
    score = torch.matmul(query, key.permute(0, 1, 3, 2))
...
def _sfdp_pattern_24(query, key, value, attn_mask):
    return _sfdp_pattern_t5_v2(query, key, value, attn_mask, do_view=True)

def _sfdp_pattern_25(query, key, value, attn_mask):
    return _sfdp_pattern_t5_v2(query, key, value, attn_mask, do_view=False)

def _sfdp_replacement_23(query, key, value, attn_mask):
    counters["inductor"]["fuse_attention"] += 1
    query = query.permute(0, 2, 1, 3)
    key = key.permute(0, 2, 1, 3)
    value = value.permute(0, 2, 1, 3)
...
#     # for T5 with inplace/outplace add and
...
def _sfdp_pattern_t5_v3(query, key, value, do_view):
    # for T5 with/without view and
    # return key and value and
    # attn_mask is generated by atem.full(..., 0)
    query = query.permute([0, 2, 1, 3])
    key = key.permute([0, 2, 1, 3])
...
def _sfdp_pattern_26(query, key, value):
    return _sfdp_pattern_t5_v3(query, key, value, do_view=True)

def _sfdp_pattern_27(query, key, value):
    return _sfdp_pattern_t5_v3(query, key, value, do_view=False)

def _sfdp_replacement_24(query, key, value):
    counters["inductor"]["fuse_attention"] += 1
    query = query.permute(0, 2, 1, 3)
    key = key.permute(0, 2, 1, 3)
    value = value.permute(0, 2, 1, 3)
