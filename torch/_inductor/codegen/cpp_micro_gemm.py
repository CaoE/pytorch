...
{%- endif %}
    ...
    std::cout << "bf16 config: " << std::chrono::duration_cast<res>(t2 - t1).count() << " ns" << std::endl;
...
def check_brgemm_extra(config, m, n, k, alpha, num_threads):
    assert (config.input_dtype == torch.half or config.input_dtype == torch.bfloat16) and config.output_dtype == torch.float
    vnni_size = 2
...
{{declare_kernel}} {
    static struct brgemm_list {
        ...
        at::native::cpublas::GemmHelper* gemmhelpers[8];
        int64_t Ms[8];
        int64_t Ns[8];
        int64_t Ks[8];
        int64_t ldas[8];
    ...
    if (brgemm == nullptr) {
        brgemm = at::native::cpublas::brgemm_create(M, N, K,
                                                    ...
                                                    {{aten_input2_t}},
                                                    {{aten_output_t}});
        if (accum && accum_brgemms.index < 8) {
