...
void list_files_recursive(const std::string& root, std::vector<std::string>& out) {
  ...
  while (!stack.empty()) {
    ...
    while ((ent = readdir(dir)) != nullptr) {
      ...
      std::string full = cur + k_separator + name;
      struct stat st{};
      if (lstat(full.c_str(), &st) == 0) {
        if (S_ISDIR(st.st_mode)) {
          stack.push_back(full);
        } else {
          out.push_back(normalize_path_separator(full));
        }
      }
    }
    closedir(dir);
...
std::string create_temp_dir() {
  ...
  if (const char* fixed = std::getenv("AOTI_SHARED_PATH"); fixed && *fixed) {
    std::string base = normalize_path_separator(fixed);
    if (!base.empty() && base.back() == '/') {
      base.pop_back();
    }
    temp_dir = base + "/XXXXXX";
    temp_dir = normalize_path_separator(temp_dir);
  }
...
AOTIModelPackageLoader::AOTIModelPackageLoader(
  ...
  std::vector<std::string> obj_filenames;
  std::string file_prefix; // zip 前缀（共享目录模式下可为空）
  ...
  if (is_directory(model_package_path) && !is_zip_file(model_package_path)) {
    ...
    // 对应打包展开后的结构: data/aotinductor/<model_name>/...
    model_directory = normalize_path_separator(
        temp_dir_ + k_separator + "data" + k_separator + "aotinductor" + k_separator + model_name);
