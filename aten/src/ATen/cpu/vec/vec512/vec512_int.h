...
template<typename T>
Vectorized<int32_t> inline convert_to_int32(const T* ptr, int count=Vectorized<int32_t>::size()) {
  return Vectorized<int32_t>::loadu(ptr, count);
}

template<>
Vectorized<int32_t> inline convert_to_int32<int8_t>(const int8_t* ptr, int count=Vectorized<int32_t>::size()) {
  if (count == Vectorized<int32_t>::size()) {
    ...
    return _mm512_cvtepi8_epi32(_mm512_castsi512_si128(a.values));
  } 
}

template<>
Vectorized<int32_t> inline convert_to_int32<uint8_t>(const uint8_t* ptr, int count=Vectorized<int32_t>::size()) {
  if (count == Vectorized<int32_t>::size()) {
    return _mm512_cvtepu8_epi32(_mm_loadu_si128(reinterpret_cast<const __m128i*>(ptr)));
  } else {
    auto a = Vectorized<uint8_t>::loadu(ptr, count);
    return _mm512_cvtepu8_epi32(_mm512_castsi512_si128(a.values));
  }
}
