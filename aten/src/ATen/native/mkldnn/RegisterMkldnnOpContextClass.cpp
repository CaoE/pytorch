...
TORCH_LIBRARY(mkldnn, m) {
  ...
  m.def(TORCH_SELECTIVE_SCHEMA(
      "mkldnn::mkldnn_convolution_with_out_stride(Tensor(a!) Y, Tensor X, Tensor W, Tensor? B, int[] padding, int[] stride, int[] dilation, int[] out_strides, int groups, str attr, Scalar?[] scalars, str? algorithm) -> Tensor(a!)"));
